...
public:
    ...
    */
    ...
    /** Function to encode a message that is input as a string of
        lower case letters and space characters
        @param message The input message as a string of
        @return The coded message as a string of 1 and 0 characters
    */
    std::string encode(const std::string& message) const;

    /** Starter function to perform breadth-first traversal
        of the Huffman tree and build the code table.
        This is not directly required by the exercise, but is implied.
    */
    void build_code() {
        codes.clear();
        codes.resize(27);
        build_code(huff_tree, "");
    }
